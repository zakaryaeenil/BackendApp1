...
namespace NejPortalBackend.Application.Dossiers.Queries.CientGetDossiers;

[Authorize(Roles = Roles.Client)]
public record CientGetDossiersQuery : IRequest<PaginatedList<DossierDto>>
{
...
public class CientGetDossiersQueryValidator : AbstractValidator<CientGetDossiersQuery>
{
    public CientGetDossiersQueryValidator()
    {
...
public class CientGetDossiersQueryHandler : IRequestHandler<CientGetDossiersQuery, PaginatedList<DossierDto>>
{
    private readonly IApplicationDbContext _context;
    private readonly IUser _currentUserService;
    private readonly IIdentityService _identityService;
    private readonly ILogger<CientGetDossiersQueryHandler> _logger;

    public CientGetDossiersQueryHandler(IIdentityService identityService, IApplicationDbContext context, IUser currentUserService, ILogger<CientGetDossiersQueryHandler> logger)
    {
        _context = context;
    ...
    public async Task<PaginatedList<DossierDto>> Handle(CientGetDossiersQuery request, CancellationToken cancellationToken)
    {
        _logger.LogInformation("CientGetDossiersQueryHandler received with parameters: {Request}", request);
        ...
        try
        {
            ...
            _logger.LogInformation("Successfully handled CientGetDossiersQueryHandler.");
            return result;
        }
        catch (UnauthorizedAccessException ex)
        {
            _logger.LogWarning(ex, "Unauthorized access in CientGetDossiersQueryHandler.");
            throw;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "An error occurred while handling CientGetDossiersQueryHandler.");
            throw;
        }
    }
}
